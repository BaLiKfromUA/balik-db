...
pub trait Storage {
    ...
    /// SQL `SELECT` reads through the vectorized [`Storage::scan_batches`]
    /// instead; this rid-carrying form backs the `UPDATE` and `DELETE` executors,
    /// which must visit rows by rid, and storage tests asserting rid-level
    /// read-back.
