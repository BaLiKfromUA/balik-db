...
///
/// Runs the pipeline: parse → bind → (optimize) → lower → execute, applying
/// logical optimizations only when `optimize` is set. A SELECT prints its rows
/// as a table; CREATE TABLE / INSERT print an effect line.
///
